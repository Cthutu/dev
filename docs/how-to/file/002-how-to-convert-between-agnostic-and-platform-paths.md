...
1. Call `file_init()`.
2. Use `path_from_platform()` when a path comes from the OS or another library.
3. Use `path_to_platform()` when you need a host path for syscalls or external APIs.
4. Treat those two functions as the platform boundary.
5. Call `file_done()` when finished.
